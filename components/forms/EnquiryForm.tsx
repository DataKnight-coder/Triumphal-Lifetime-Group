...
export default function EnquiryForm({ siteKey = "", initialService = "", compact = false, onSuccess }: EnquiryFormProps) {
  ...
  if (state === "success") return <div role="status" aria-live="polite" className="border border-green-300 bg-green-50 rounded-xl p-7 text-green-900"><h3 className="font-serif text-2xl mb-2">Request received</h3><p>{message}</p></div>;
  return <form onSubmit={submit} aria-busy={state === "sending"} className={compact ? "space-y-4" : "space-y-6"}>
    ...
    {message && <p role="alert" aria-live="assertive" className={state === "error" ? "text-sm text-red-700" : "text-sm"}>{message}</p>}
