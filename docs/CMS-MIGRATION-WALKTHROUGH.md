...
Then deploy the frontend and let the WordPress publishing hook rebuild Netlify. Verify `/disclaimer` has one ordered set of sections, the page title and summary follow CMS edits, the last-updated label appears, the footer and contact page show `admin@triumphallifetimegroup.com` and `+234 903 186 5491`, and the Nigeria link opens `https://wa.me/2349031865491`. Send one test enquiry to verify the mailbox destination. Do not run the full `cms:migrate -- --apply` against edited production content: it upserts the entire retained source inventory and could overwrite newer CMS edits.
