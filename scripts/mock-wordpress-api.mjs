...
const flexiblePages = [{ path:"sample-wordpress-page", title:"Sample WordPress Page", summary:"A page created in the standard WordPress editor.", body:"<h2>Editable section</h2><p>This content comes from WordPress.</p>", image:null, modifiedAt:"2026-09-13T00:00:00Z" }];
const mapped = {
...
const server = http.createServer((request, response) => {
  ...
  else if (url.pathname === "/wp-json/tlg/v1/locations") payload = locations.filter((item) => item.status === "active").map((item) => ({ name:item.title, slug:item.slug, description:item.description || "", country:item.country || "", city:item.city || "", publicLabel:item.public_label || "", address:item.address || "", clientFacing:item.client_facing === true, operationalStatus:item.operational_status, services:item.services_available || [], email:item.public_email || "", phone:item.public_phone || "", image:item.image || null, order:Number(item.display_order || 0) }));
